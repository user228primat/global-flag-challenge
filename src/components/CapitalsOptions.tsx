...
const CapitalsOptions = () => {
  ...
  // Verify that the category exists in gameCategories before proceeding
  if (!gameCategories[capitalsCategoryId]) {
    console.error(`Category ${capitalsCategoryId} not found in gameCategories`);
    toast({
      title: "Ошибка",
      description: `Категория ${capitalsCategoryId} не найдена в данных игры`,
      variant: "destructive",
    });
    navigate('/capitals');
    return null;
