...
const CapitalsOptions = () => {
  ...
  const getCapitalsCategoryId = (region: string): CategoryId | null => {
    ...
    console.log("Region mapping input:", region);
    console.log("Mapping result:", capitalsMapping[region]);
  ...
  if (!capitalsCategoryId) {
    console.error(`No matching capitals category for region: ${regionId}`);
    toast({
      title: "Ошибка",
      description: `Категория столиц не найдена для региона ${regionId}`,
      variant: "destructive",
    });
    navigate('/capitals');
    return null;
  ...
  return (
    <div className="min-h-screen bg-background pb-16 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-40 -right-20 w-96 h-96 rounded-full bg-blue-600/10 blur-[120px] opacity-70 animate-float"></div>
        <div className="absolute bottom-40 -left-20 w-108 h-108 rounded-full bg-indigo-500/10 blur-[130px] opacity-60 animate-float" style={{ animationDelay: '-4s' }}></div>
        <div className="absolute top-1/4 left-1/4 w-72 h-72 rounded-full bg-purple-500/5 blur-[80px] opacity-40 animate-float" style={{ animationDelay: '-2s' }}></div>
      </div>

      {/* Content container with improved back button */}
      <div className="w-full max-w-xl mx-auto px-4 pt-6 relative z-20">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-bold mb-3 text-white text-shadow bg-clip-text text-transparent bg-gradient-to-r from-blue-200 to-blue-100">
            {displayName}
          </h1>
